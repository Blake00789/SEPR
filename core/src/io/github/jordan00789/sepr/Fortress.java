...
public class Fortress extends Entity implements Attack {
	...
	public void attack(Entity e, int n) {
		if (e != null) {
			if (ableToAttack) {
				// * 10 means the projectile is created 10 pixels away from the fortress
	...
	public void update(float delta) {
		if (distanceTo(MainGame.currentTruck) < 100f) {
			...
			case 2:
				...
				break;
			case 3:
				// Octagonal spread around the fortress
		...
		// Waits until the last attack has finished
		if (goos.size() < 1) {
			ableToAttack = true;
		}
	}
	...
	public float directionTo(float x, float y) {
		//384 is used because it centres onto the centre of the fortress
